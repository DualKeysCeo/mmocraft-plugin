...
import HealthBarAPI.BarHideEvent;
...
public class PlayerBar {
	...
	public static void setupBelow() {
		...
		if (playerEnabled && useBelow) {
			...
			belowObj.setDisplayName(Utils.replaceSymbols(instance.getConfig().getString(HealthBarConfig.Nodes.PLAYERS_BELOW_TEXT.getNode())));
	...
	public static void loadConfiguration() {
		...
		textMode = 				config.getBoolean(HealthBarConfig.Nodes.PLAYERS_AFTER_TEXT_MODE.getNode());
		useCustomBar = 			config.getBoolean(HealthBarConfig.Nodes.PLAYERS_AFTER_USE_CUSTOM.getNode());
		useBelow = 				config.getBoolean(HealthBarConfig.Nodes.PLAYERS_BELOW_ENABLE.getNode());
		belowUseProportion = 	config.getBoolean(HealthBarConfig.Nodes.PLAYERS_BELOW_USE_PROPORTION.getNode());
		belowNameProportion = 	config.getInt(HealthBarConfig.Nodes.PLAYERS_BELOW_PROPORTIONAL_TO.getNode());
		belowUseRawAmountOfHearts = 	config.getBoolean(HealthBarConfig.Nodes.PLAYERS_BELOW_DISPLAY_RAW_HEARTS.getNode());
		...
		if (useCustomBar) {
			...
			PlayerBarUtils.create10DefaultTeams(sb, config.getInt(HealthBarConfig.Nodes.PLAYERS_AFTER_STYLE.getNode()));
		}
