package ml.loganhouston.mmocraft.healthbars;
...
import ml.loganhouston.mmocraft.Main;
import utils.MobBarsUtils;
import utils.Utils;
...
import com.avaje.ebeaninternal.server.type.ScalarTypeBoolean.StringBoolean;
...
public class DamageListener implements Listener {
	...
	private static		BarTypes barStyle;
 ...
 */
	...
	private static void showMobHealthBar (final LivingEntity mob) {
		...
		scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
	    	  public void run() {
			      ...
			      else if (barStyle == BarTypes.DEFAULT_TEXT)
			      {
	...
	public static void loadConfiguration() {
		...
		//setup mobs
		mobEnabled = 			config.getBoolean(HealthBarConfig.Nodes.MOB_ENABLE.getNode());
		mobUseText = 			config.getBoolean(HealthBarConfig.Nodes.MOB_TEXT_MODE.getNode());
		mobUseCustomText = 		config.getBoolean(HealthBarConfig.Nodes.MOB_CUSTOM_TEXT_ENABLE.getNode());
		mobCustomText = 		Utils.replaceSymbols(config.getString(HealthBarConfig.Nodes.MOB_CUSTOM_TEXT.getNode()));
		mobSemiHidden = 		config.getBoolean(HealthBarConfig.Nodes.MOB_SHOW_IF_LOOKING.getNode());
		
		mobHideDelay = (long) 	config.getInt(HealthBarConfig.Nodes.MOB_DELAY.getNode())*20;
		if (config.getBoolean(HealthBarConfig.Nodes.MOB_ALWAYS_SHOWN.getNode(), false)) {
			mobHideDelay = 0L;
		}
		
		mobUseCustomBar = 		config.getBoolean(HealthBarConfig.Nodes.MOB_USE_CUSTOM.getNode());
		showOnCustomNames = 	config.getBoolean(HealthBarConfig.Nodes.MOB_SHOW_ON_NAMED.getNode());
		mobUseDisabledWorlds = config.getBoolean(HealthBarConfig.Nodes.MOB_WORLD_DISABLING.getNode());

		if (mobUseDisabledWorlds) {
			mobDisabledWorlds = Arrays.asList(plugin.getConfig()
												.getString(HealthBarConfig.Nodes.MOB_DISABLED_WORLDS.getNode())
												.toLowerCase()
												.replace(" ", "")
												.split(","));
		}
		
		mobTypeDisabling =		config.getBoolean(HealthBarConfig.Nodes.MOB_TYPE_DISABLING.getNode());
		
		
		
		//setup players
		playerEnabled = config.getBoolean(HealthBarConfig.Nodes.PLAYERS_ENABLE.getNode());
		
		playerHideDelay = (long) config.getInt("player-bars.after-name.hide-delay-seconds")*20;
		if (config.getBoolean(HealthBarConfig.Nodes.PLAYERS_AFTER_ALWAYS_SHOWN.getNode(), false)) {
			playerHideDelay = 0L;
		}
		
		
		playerUseAfter = config.getBoolean(HealthBarConfig.Nodes.PLAYERS_AFTER_ENABLE.getNode());
		
		//setup for epicboss
		hookEpicboss = config.getBoolean(HealthBarConfig.Nodes.HOOKS_EPIBOSS.getNode());
		...
		//text - maybe custom - medium priority on configs
			...
			if (mobUseCustomText) {
				...
				barStyle = BarTypes.CUSTOM_TEXT;
			} else {
				barStyle = BarTypes.DEFAULT_TEXT;
			}
		} else {
		...
		if (mobTypeDisabling) {
			mobDisabledTypes = Utils.getTypesFromString(config.getString(HealthBarConfig.Nodes.MOB_DISABLED_TYPES.getNode()));
		}
