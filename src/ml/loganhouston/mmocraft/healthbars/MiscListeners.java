...
public class MiscListeners implements Listener {
	...
	public void onInteract(PlayerInteractEntityEvent event) {
		...
		if (event.getPlayer().getItemInHand().getType() == Material.NAME_TAG && entity instanceof LivingEntity) {
			...
			if (DamageListener.mobHideDelay == 0L && HealthBarAPI.HealthBarAPI.mobHasBar(mob)) {
		...
		if (entity instanceof Villager) {
			...
			if (villager.isAdult() && HealthBarAPI.HealthBarAPI.mobHasBar(villager)) {
			...
			if (horse.getVariant() == Variant.DONKEY || horse.getVariant() == Variant.MULE) {
				...
				if (HealthBarAPI.HealthBarAPI.mobHasBar(horse)) {
...
//			the horse may have an inventory (opening is client side), and it doesn't display the name when ridden...so hide it!
	...
	public static void loadConfiguration() {
		...
		usePlayerPermissions = config.getBoolean(HealthBarConfig.Nodes.USE_PLAYER_PERMISSIONS.getNode());
		fixTabNames = config.getBoolean(HealthBarConfig.Nodes.FIX_TAB_NAMES.getNode());
		playerHideDelay = config.getInt(HealthBarConfig.Nodes.PLAYERS_AFTER_DELAY.getNode());
		...
		playerUseDisabledWorlds = config.getBoolean(HealthBarConfig.Nodes.PLAYERS_WORLD_DISABLING.getNode());
		
		overrideOtherScoreboards = config.getBoolean(HealthBarConfig.Nodes.OVERRIDE_OTHER_SCOREBOARD.getNode());
		if (playerUseDisabledWorlds) {
			playerDisabledWorlds = Arrays.asList(
					instance.getConfig()
					.getString(HealthBarConfig.Nodes.PLAYERS_DISABLED_WORLDS.getNode())
					.toLowerCase()
					.replace(" ", "")
					.split(","));
		}
